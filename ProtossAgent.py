...
#step() method lets determine the base position based on the location of our Nexus
def step(self, obs):
    super(RawAgent, self).step(obs)
    ...
    if len(pylons) == 0 and obs.observation.players.minerals >= 100:
        ...
        if len(probes) > 0:
            ...
            probe = probes[np.argmin(distances)]    # extract the probe with the shortest distance 
            return actions.RAW_FUNCTIONS.Build_Pylon_pt("now",probe.tag, pylon_xy) # tell the probe to build the pylon 
